...
(function ($) {
    ...
    $.zxxbox = function (elements, options) {
        ...
        var s = $.extend({}, zxxboxDefault, options || {});
        //弹框的显示
        var WRAP = '<div id="zxxBlank" onselectstart="return false;"></div><div class="wrap_out" id="wrapOut"><div class="wrap_in" id="wrapIn"><div id="wrapBar" class="wrap_bar"  onselectstart="return false;"><div class="wrap_title"><span>' + s.title + '</span></div><a href="javasctipt:" class="wrap_close"  title="关闭"  id="wrapClose">' + s.shut + '</a></div><div class="wrap_body" id="wrapBody"></div></div></div>';
        if ($("#wrapOut").size()) {
            $("#wrapOut").show();
            if (s.bg) {
                $("#zxxBlank").show();
    ...
    $.extend($.zxxbox, {
        ...
        dialog: function (content, options) {
            var element = $('<div class="wrap_remind">' + content + '</div>');
            $.zxxbox(element, options);
        },
        //将页面存在的元素集合传递给dialog
        content: function (DomId, options) {
            var _el = $("#" + DomId).html();
            $("#" + DomId).remove();
            var el = $('<div class="wrap_remind">' +_el + '</div>');
            $.zxxbox(el, options);
        }
    });
    var zxxboxDefault = {
