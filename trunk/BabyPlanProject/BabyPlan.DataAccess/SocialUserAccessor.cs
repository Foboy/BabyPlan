...
namespace BabyPlan.DataAccess
{
    public class SocialUserAccessor
    {
        ...
        private SocialUserAccessor()
        {
            ...
            cmdGetSocialUser = new MySqlCommand(" select social_user_type,access_token,access_token_secret,uid,user_id,social_user_id from social_user where (@uid='' or uid = @uid)  and (@sutype=0 or social_user_type = @sutype) and (@user_id=0 or user_id=@user_id)");
            cmdGetSocialUser.Parameters.Add("@uid", MySqlDbType.String);
            cmdGetSocialUser.Parameters.Add("@sutype", MySqlDbType.Int32);
            cmdGetSocialUser.Parameters.Add("@user_id", MySqlDbType.Int32);
            #endregion
        }
