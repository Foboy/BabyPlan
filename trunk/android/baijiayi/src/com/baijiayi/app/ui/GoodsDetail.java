...
import com.baijiayi.app.adapter.GridViewFaceAdapter;
import com.baijiayi.app.adapter.ListViewGoodsCommentAdapter;
import com.baijiayi.app.bean.CommentModel;
import com.baijiayi.app.bean.GoodsCommentList;
...
import com.baijiayi.app.widget.GoodsDetailPhotoListLayout;
...
public class GoodsDetail extends Activity {
	...
	private ListViewGoodsCommentAdapter lvCommentAdapter;
	private List<CommentModel> lvCommentData = new ArrayList<CommentModel>();
	...
	private GoodsDetailPhotoListLayout mPhotoLayout;
	private HorizontalScrollView mPhtotsContainer;
    
    private GoodsModel goodsDetail;
    ...
    {    	
		curId = getIntent().getIntExtra("goods_id", 0);
		curCatalog = GoodsCommentList.CATALOG_GOODS;
    	
    	if(curId > 0) tempCommentKey = AppConfig.TEMP_COMMENT + "_" + curCatalog + "_" + curId;
    	
    	mBack = (ImageView)findViewById(R.id.goods_detail_back);
    	mRefresh = (ImageView)findViewById(R.id.goods_detail_refresh);
    	mLinearlayout = (LinearLayout)findViewById(R.id.goods_detail_linearlayout);
    	mProgressbar = (ProgressBar)findViewById(R.id.goods_detail_head_progress);
    	...
    	mPhoneLayout = (LinearLayout)findViewById(R.id.goods_user_phone_linear);
    	mQQLayout = (LinearLayout)findViewById(R.id.goods_user_qq_linear);
    	
    	phone = (TextView)findViewById(R.id.goods_user_phone);
    	qq = (TextView)findViewById(R.id.goods_user_qq);
    	
    	lvHeader = View.inflate(this, R.layout.goods_detail_content, null);
    	userface = (ImageView)lvHeader.findViewById(R.id.goods_listitem_userface);
    	username = (TextView)lvHeader.findViewById(R.id.goods_listitem_username);
    	date = (TextView)lvHeader.findViewById(R.id.goods_listitem_date);
    	commentCount = (TextView)lvHeader.findViewById(R.id.goods_listitem_commentCount);
    	//image = (ImageView)lvHeader.findViewById(R.id.goods_listitem_pic);
    	price=(TextView)lvHeader.findViewById(R.id.goods_detail_price);
    	
    	mPhtotsContainer = (HorizontalScrollView)lvHeader.findViewById(R.id.goods_photos_container); 

		mPhotoLayout = (GoodsDetailPhotoListLayout)lvHeader.findViewById(R.id.goods_photos_Layout);
    	
    	content = (TextView)lvHeader.findViewById(R.id.goods_listitem_content);
    	...
    	lvCommentAdapter = new ListViewGoodsCommentAdapter(this, lvCommentData, R.layout.comment_listitem); 
    	mLvComment = (PullToRefreshListView)findViewById(R.id.goods_detail_commentlist);
...
/*        mLvComment.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		if(view instanceof TextView){
        			com = (CommentModel)view.getTag();
        		}else{
            		ImageView img = (ImageView)view.findViewById(R.id.comment_listitem_userface);
            		com = (CommentModel)img.getTag();
        		} 
        		if(com == null) return;        		
        		
        		//跳转--回复评论界面
        		UIHelper.showGoodsCommentReply(GoodsDetail.this,curId, com.getId(), com.getAuthor().getName(), com.getContent());
        	}
		});*/
	...
	{		
    	...
    	mHandler = new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if(msg.what == 1){
					username.setText(goodsDetail.getAuthor().getName());
			    	username.setOnClickListener(faceClickListener);
					date.setText(StringUtils.friendly_time(goodsDetail.getCreateTime()));
					commentCount.setText(goodsDetail.getReplyNum()+"");
					price.setText(goodsDetail.getPrice());
					content.setText(goodsDetail.getDescription());
					
					String phoneStr=goodsDetail.getPhone();
					if(!phoneStr.contains("手机") && !StringUtils.isEmpty(phoneStr))
					{
						mPhoneLayout.setVisibility(View.VISIBLE);
						phone.setText(phoneStr);
					}
					String qqStr=goodsDetail.getQQ();
					if(!qqStr.contains("QQ") && !StringUtils.isEmpty(qqStr))
					{
						mQQLayout.setVisibility(View.VISIBLE);
						qq.setText(qqStr);
					}
					mPhotoLayout.setPhotoList(goodsDetail.getPics(),appContext);
					//加载用户头像
					String faceURL = goodsDetail.getAuthor().getPic().getSPicUrl();
					...
					//加载图片
/*					String imgSmall = goodsDetail.getPic().getPicUrl();
					...
					//发送通知广播
					if(msg.obj != null){
						UIHelper.sendBroadCast(GoodsDetail.this, (Notice)msg.obj);
					}
				}else if(msg.what == 0){
						UIHelper.ToastMessage(GoodsDetail.this, R.string.msg_load_is_null);	
				}else{
					((AppException)msg.obj).makeToast(GoodsDetail.this);
				}
			}
		};
    	this.loadGoodsDetail(curId, mHandler, false);
    	...
    	mCommentHandler = new Handler()
		{
			...
			public void handleMessage(Message msg) {
				...
				if(msg.what >= 0){						
					GoodsCommentList list = (GoodsCommentList)msg.obj;
					...
					case UIHelper.LISTVIEW_ACTION_SCROLL:
						...
						if(lvCommentData.size() > 0){
							for(CommentModel com1 : list.getCommentlist()){
								boolean b = false;
								for(CommentModel com2 : lvCommentData){
					...
					//发送通知广播
					if(notice != null){
						UIHelper.sendBroadCast(GoodsDetail.this, notice);
					}
				}				
				else if(msg.what == -1){
					//有异常--也显示更多 & 弹出错误消息
				...
				if(lvCommentData.size()==0){
					...
					lvComment_foot_more.setText("暂无评论");
				}
		...
		new Thread(){
			...
			public void run() {
				...
				if(action == UIHelper.LISTVIEW_ACTION_REFRESH || action == UIHelper.LISTVIEW_ACTION_SCROLL)
					isRefresh = true;
				try {
					GoodsCommentList commentlist = ((AppContext)getApplication()).getGoodsCommentList(id, pageIndex, isRefresh);				
	...
	private void loadGoodsDetail(final int goodsId, final Handler handler, final boolean isRefresh){
		...
		new Thread(){
			...
			public void run() {
				Message msg = new Message();
				try {
					goodsDetail = ((AppContext)getApplication()).getGoods(goodsId, isRefresh);
					msg.what = (goodsDetail!=null && goodsDetail.getId()>0) ? 1 : 0;
					msg.obj = (goodsDetail!=null) ? goodsDetail.getNotice() : null;
				} catch (AppException e) {
	            	e.printStackTrace();
	            	msg.what = -1;
	            	msg.obj = e;
	...
	{ 	
        ...
        { 
        	CommentModel comm = (CommentModel)data.getSerializableExtra("COMMENT_SERIALIZABLE");
    ...
    private View.OnClickListener refreshClickListener = new View.OnClickListener() {
		@Override
		public void onClick(View v) {	
			loadGoodsDetail(curId, mHandler, true);
			loadLvCommentData(curId,curCatalog,0,mCommentHandler,UIHelper.LISTVIEW_ACTION_REFRESH);
		}
	};
	...
	private View.OnClickListener faceClickListener = new View.OnClickListener(){
		...
		public void onClick(View v) {
			//if(goodsDetail != null)
			//	UIHelper.showUserCenter(v.getContext(), goodsDetail.getGoods().getAuthor().getId(), goodsDetail.getGoods().getAuthor().getName());
		}
	};
	...
	private View.OnClickListener imageClickListener = new View.OnClickListener(){
		...
		public void onClick(View v) {
			if(goodsDetail!=null)
				UIHelper.showImageZoomDialog(v.getContext(), goodsDetail.getPic().getBPicUrl());
		}
	};
	...
	private View.OnClickListener commentpubClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {	
			...
			if(!ac.isLogin()){
				UIHelper.showLoginDialog(GoodsDetail.this);
				return;
			}
			...
			final Handler handler = new Handler(){
				...
				public void handleMessage(Message msg) {
					...
					if(msg.what == 1 && msg.obj != null){
						...
						UIHelper.ToastMessage(GoodsDetail.this, res.getErrorMessage());
						if(res.OK()){
							//发送通知广播
							if(res.getNotice() != null){
								UIHelper.sendBroadCast(GoodsDetail.this, res.getNotice());
							}
							//恢复初始底部栏
							mFootViewSwitcher.setDisplayedChild(0);
