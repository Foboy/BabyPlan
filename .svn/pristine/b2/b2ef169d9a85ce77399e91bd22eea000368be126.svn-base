...
public class User extends Base {
	...
	public void setResult(AdResult result) {
		this.result = result;
	}
	...
	public static User parse(String itemResult)throws IOException,
	AppException {
		User user = new User();
		Result res = null;
		UserModel um = null;
        try { 
        	 AdResult result = AdResult.parse(itemResult);
        	 res = new Result();
        	 res.setErrorCode(result.OK()?1:-1);
        	 res.setErrorMessage(result.getErrorMessage());
        	 user.setValidate(res);
        	 user.setResult(result);
			if (result.OK()) {
				JSONObject userJSON = result.getData();
				if (userJSON != null) {
					um = UserModel.parse(userJSON);
					user.setUser(um);
					user.setQq(um.getQQ());
					user.setPhone(um.getPhone());
					user.setAccount(um.getAccount());
					user.setName(um.getName());
					user.setUid(um.getId());
					PicModel head = um.getPic();
					if(head!=null)
					user.setFace(head.getSPicUrl());
				}
			}
        } catch (Exception e) {
