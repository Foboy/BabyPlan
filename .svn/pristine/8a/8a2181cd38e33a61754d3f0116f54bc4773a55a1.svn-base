...
import com.baijiayi.app.bean.BlogCommentList;
import com.baijiayi.app.bean.BlogList;
import com.baijiayi.app.bean.CommentList;
import com.baijiayi.app.bean.FavoriteList;
import com.baijiayi.app.bean.FriendList;
...
import com.baijiayi.app.bean.MyInformation;
...
import com.baijiayi.app.bean.SearchList;
import com.baijiayi.app.bean.Software;
import com.baijiayi.app.bean.SoftwareCatalogList;
import com.baijiayi.app.bean.SoftwareList;
import com.baijiayi.app.bean.Tweet;
import com.baijiayi.app.bean.TweetList;
import com.baijiayi.app.bean.URLs;
import com.baijiayi.app.bean.Update;
import com.baijiayi.app.bean.UploadFileItemModel;
import com.baijiayi.app.bean.User;
import com.baijiayi.app.bean.UserInformation;
...
public class ApiClient {
	...
	private static String getUserAgent(AppContext appContext) {
		if(appUserAgent == null || appUserAgent == "") {
			StringBuilder ua = new StringBuilder("360baijiayi.com");
	...
	private static String http_get_string(AppContext appContext, String url) throws AppException {	
		//System.out.println("get_url==> "+url);
		String cookie = getCookie(appContext);
		String userAgent = getUserAgent(appContext);
	...
	private static String _post_string(AppContext appContext, String url, Map<String, Object> params, Map<String,File> files) throws AppException {
		//System.out.println("post_url==> "+url);
		String cookie = getCookie(appContext);
		String userAgent = getUserAgent(appContext);
	...
	private static AdResult http_post_string(AppContext appContext, String url, Map<String, Object> params, Map<String,File> files) throws AppException, IOException {
        return AdResult.parse(_post_string(appContext, url, params, files));  
	}
	...
	public static Update checkVersion(AppContext appContext) throws AppException {
		try{
			return Update.parse(http_get(appContext, URLs.BUPDATE_VERSION));		
		}catch(Exception e){
	 ...
	 * @throws AppException
	 */
	public static User login(AppContext appContext, String username, String pwd,boolean getUserInfo) throws AppException {
...
/*		if(appContext.isHttpsLogin()){
		...
		try{
			return User.parse(_post_string(appContext, loginurl, params, null));		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static User register(AppContext appContext, String username, String pwd,int sex,boolean getUserInfo) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("username", username);
		params.put("password", pwd);
		params.put("sex", sex);
		...
		String loginurl = URLs.REGISTER_URL;
/*		if(appContext.isHttpsLogin()){
			loginurl = URLs.LOGIN_VALIDATE_HTTPS;
		}*/
	...
	public static GoodsItemList getGoodsList(AppContext appContext, final int cat, final int pageIndex, final int pageSize) throws AppException {
		String newUrl = _MakeURL(URLs.GOODS_LIST, new HashMap<String, Object>(){{
			put("itemType", cat);
			put("page", pageIndex);
			put("pageSize", pageSize);
		}});
		//page: page, age: age, itemType: itemType, sexType: sexType
		try{
			return GoodsItemList.parse(http_get_string(appContext, newUrl));		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static GoodsModel getGoodsDetail(AppContext appContext, final int goods_id) throws AppException {
		String newUrl = _MakeURL(URLs.GOODS_DETAIL, new HashMap<String, Object>(){{
			put("id", goods_id);
		}});
		try{
			return GoodsModel.parse(http_get_string(appContext, newUrl));			
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static UploadFileItemModel pubPic(AppContext appContext, File pubPic) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		Map<String, File> files = new HashMap<String, File>();
		params.put("type", "g");
		params.put("width", 470);
		params.put("height", 470);
		files.put("img", pubPic);
		
		try{
			return UploadFileItemModel.parse(_post_string(appContext, URLs.PUB_GOODS_PIC, params, files)); 	
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static AdResult pubGoods(AppContext appContext, GoodsModel goods) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
/*		params.put("uid", tweet.getAuthorId());
		params.put("msg", tweet.getBody());*/
		...
		try
		{
			...
			goodsJSON.put("Description", goods.getDescription());
			goodsJSON.put("Phone", goods.getPhone());
			goodsJSON.put("QQ", goods.getQQ());
			goodsJSON.put("IsWash", goods.getIsWash());
			goodsJSON.put("Price", goods.getDoublePrice());
			goodsJSON.put("Age", goods.getAge());
			goodsJSON.put("ItemType", goods.getItemType());
			goodsJSON.put("Sex", goods.getSex());
			JSONArray items = new JSONArray();
			for (PicModel item : goods.getPics()) {
				UploadFileItemModel picResult = pubPic(appContext,item.getPubPic());
				if (picResult.getSaved() && picResult.getPicId() > 0) {

					JSONObject ji = new JSONObject();
					ji.put("PicId", picResult.getPicId());
					ji.put("Id", item.getId());
					items.put(ji);
				}
			}
			goodsJSON.put("Pics", items);
			String data = goodsJSON.toString();
			params.put("goods", data);
...
/*		if(tweet.getImageFile() != null)
		...
		try{
			return http_post_string(appContext, URLs.GOODS_PUB, params, files);		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static AdResult userSetSave(AppContext appContext, File head,String qq,String phone) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("qq", qq);
		...
		if(head !=null)
		{
			UploadFileItemModel picResult = pubPic(appContext,head);
			if (picResult.getSaved() && picResult.getPicId() > 0) {
				params.put("head", picResult.getPicId());
			}
		}
		...
		try{
			return http_post_string(appContext, URLs.USER_SET, params, files);		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	public static GoodsCommentList getGoodsCommentList(AppContext appContext, final int id, final int pageIndex, final int pageSize) throws AppException {
		String newUrl = _MakeURL(URLs.GOODSCOMMENT_LIST+id, new HashMap<String, Object>(){{
			put("id", id);
			put("pageIndex", pageIndex);
			put("pageSize", pageSize);
		}});
		...
		try{
			return GoodsCommentList.parse(http_get_string(appContext, newUrl));		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
